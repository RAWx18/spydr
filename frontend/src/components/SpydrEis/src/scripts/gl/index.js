...
import SpydrEmotionalCore from '../emotional-core';
...
export default new class {
  ...
  initEmotionalCore() {
    this.emotionalCore = new SpydrEmotionalCore(this.simMaterial, this.renderMaterial);
    // Set an initial neutral emotion
    this.emotionalCore.setEmotion('neutral');
    console.log('Spydr Emotional Core initialized');
  }
  
  setEmotion(emotion, intensity) {
    if (this.emotionalCore) {
      this.emotionalCore.setEmotion(emotion, intensity);
    }
  }
  ...
  render() {
    ...
    // Update the emotional core with time data
    if (this.emotionalCore) {
      this.emotionalCore.update({ delta: 16.67 }); // Approximate 60fps in milliseconds
    }
